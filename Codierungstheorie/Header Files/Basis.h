...
public:
    ...
    /*
     * Der euklidische Algorithmus ist ein Verfahren zum bestimmen des ggT (größten gemeinsamen Teilers)
     * zweier Zahlen.
     * Gegeben sind 2 Zahlen, die größere wird durch die kleinere dividiert. Falls ein Rest übrig bleibt, wird
     * die kleinere Zahl nun durch den Rest dividiert
     * Dies wird so lange fortgesetzt, bis kein Rest mehr übrig bleibt
     * zb: 16 / 12 = 1 Rest 4
     * 12 / 4 = 3 Rest 0
     * ggT = 4
     *
     * Der erweiterte euklidischer Algorithmus ermittelt eine Linearkombination des ggT für die beiden Zahlen
     * ggT(a,b) = s * a + t * b
     */
    ...
    inline int extended_euclidean_algorithm(const int a, const int b, int& s, int& t)
    {
        if (b == 0)
        {
            s = 1;
    ...
    inline void multiplicative_inverse(const int i, const int mod, int& inverse)
    {
        int s = 1;
        int t = 0;
